  ...
  return (
    <div className="antra-floating-contact fixed bottom-6 right-4 z-50 flex flex-col gap-3 items-center md:right-6 md:bottom-8 md:gap-3.5">
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            ...
            <a
              href="tel:0707079900"
              className="relative w-11 h-11 md:w-13 md:h-13 bg-[#ea580c] rounded-full shadow-[0_4px_20px_rgba(201,154,92,0.4)] flex items-center justify-center hover:scale-110 transition-transform group"
              aria-label="Gọi điện thoại"
            >
              <span className="hidden md:block absolute right-full mr-4 bg-black/90 text-white text-xs px-2.5 py-1.5 rounded opacity-0 group-hover:opacity-100 whitespace-nowrap transition-opacity pointer-events-none font-sans tracking-wide">
                Gọi 0707 079 900
              </span>
              <div className="absolute inset-0 rounded-full border border-[#ea580c] animate-[ping_1.5s_ease-in-out_infinite] opacity-75"></div>
      ...
      <button
        ...
        className="w-11 h-11 md:w-12 md:h-12 bg-white rounded-full shadow-[0_4px_16px_rgba(0,0,0,0.15)] flex items-center justify-center hover:scale-110 transition-transform cursor-pointer border border-[#ea580c]/20 z-10"
      ...
      >
        ...
        >
          <ChevronUp size={22} className="text-[#ea580c]" />
        </motion.div>
      </button>
    </div>
  );
