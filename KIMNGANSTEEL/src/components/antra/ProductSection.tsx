...
export function ProductSection() {
  ...
  return (
    <section id="products" className="py-32 bg-[#ffffff] border-b border-[#064e3b]/10 text-[#064e3b]">
      <div className="max-w-[1440px] mx-auto px-6 md:px-12 lg:px-24">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-20 gap-6">
          <div>
            ...
            <h2 className="text-3xl md:text-5xl lg:text-6xl font-light uppercase tracking-tight text-[#064e3b]">
              VẬT LIỆU XÂY DỰNG CAO CẤP
            </h2>
          </div>
          <a
            href="/san-pham"
            className="editorial-link text-xs uppercase tracking-[0.25em] text-[#064e3b] font-mono pb-1 self-start md:self-auto"
          >
            Tất Cả Quy Cách ↗
          </a>
        </div>
        ...
        <div className="flex flex-col border-t border-[#064e3b]/10">
          {products.slice(0, 4).map((prod, index) => {
            ...
            return (
              <div
                ...
                className="group relative py-10 md:py-14 border-b border-[#064e3b]/10 flex flex-col lg:flex-row lg:items-center justify-between gap-6 cursor-pointer transition-colors duration-500 hover:bg-black/[0.03] px-2 md:px-6"
              >
                <div className="flex items-baseline gap-6 md:gap-12">
                  <span className="text-xs font-mono text-[#064e3b]">0{index + 1}</span>
                  <div>
                    <h3 className="text-2xl md:text-4xl lg:text-5xl font-light uppercase tracking-tight text-[#064e3b] group-hover:text-[#064e3b] transition-colors">
                      {prod.name}
                    </h3>
                    <p className="text-xs md:text-sm text-[#064e3b] font-light mt-2 max-w-xl">
                      {prod.description}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-8 self-end lg:self-auto">
                  <div className="hidden sm:flex flex-col items-end text-xs font-mono text-[#064e3b]">
                    {prod.specs.slice(0, 2).map((spec, i) => (
                      <span key={i}>{spec}</span>
                    ))}
                  </div>
                  <span className="text-lg text-[#064e3b] opacity-0 group-hover:opacity-100 group-hover:translate-x-2 transition-all duration-300">
                    ↗
                  </span>
                </div>
                ...
                <div 
                  className={`hidden lg:block absolute right-48 top-1/2 -translate-y-1/2 w-64 h-40 pointer-events-none overflow-hidden border border-[#064e3b]/15 bg-white shadow-xl transition-all duration-700 ${
